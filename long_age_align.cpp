...
int main(int argc,char *argv[])
{
  ...
  string age_version = "LongAGE ";
#ifdef LONG_AGE_VERSION
  age_version += LONG_AGE_VERSION;
#else
  age_version += "v???";
#endif
  age_version += "\n";
  ...
  while (n_args > 0) {
	...
	if (begin == "-match=") {
	  ...
	  flag |= LongAGEaligner::SHOW_ALL_POS;
	} else if (option == "-indel") {
	  flag |= LongAGEaligner::INDEL_FLAG;
	} else if (option == "-inv") {
	  flag |= LongAGEaligner::INVERSION_FLAG;
	} else if (option == "-invl") {
	  flag |= LongAGEaligner::INVL_FLAG;
	} else if (option == "-invr") {
	  flag |= LongAGEaligner::INVR_FLAG;
	} else if (option == "-tdup") {
	  flag |= LongAGEaligner::TDUPLICATION_FLAG;
    ...
    int n_modes = 0;
    if (flag & LongAGEaligner::INDEL_FLAG)        n_modes++;
    if (flag & LongAGEaligner::TDUPLICATION_FLAG) n_modes++;
    if (flag & LongAGEaligner::INVR_FLAG)         n_modes++;
    if (flag & LongAGEaligner::INVL_FLAG)         n_modes++;
    if (flag & LongAGEaligner::INVERSION_FLAG)    n_modes++;
	...
	for (Sequence *s2 = old_seq2;s2;s2 = s2->next()) {
	  ...
	  if (both) {
	    ...
	    seq3->revcom();
      LongAGEaligner aligner1(*seq1,*seq2);
      LongAGEaligner aligner2(*seq1,*seq3);
      ...
      delete seq3;
        ...
        LongAGEaligner aligner(*seq1,*seq2);
